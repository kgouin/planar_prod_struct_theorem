...
void orient_subproblems(struct bfs_struct* b, struct tripod_decomposition_struct* t, int sp, int f1, int f2, int f3, int* acc){
	...
	if (t->v_a_next == -1 && t->v_b_next == -1){ //check to see where we hit the next tripod
		...
		for (int i = 0; i < 3; i++){ //look at all three vertices of our v_a_op triangle
			if ((acc[b->sim[t->v_a_op][i]] == acc[t->v_a] || acc[b->sim[t->v_a_op][i]] == acc[t->v_b]) && !partial_match){
				partial_match = 1;
			}
			else if ((acc[b->sim[t->v_a_op][i]] == acc[t->v_a] || acc[b->sim[t->v_a_op][i]] == acc[t->v_b]) && partial_match){
				double_match = 1;
			}
		}
		if (double_match){
		...
		if (!double_match) { //no double match. reset values and try with v_b_op
			...
			for (int i = 0; i < 3; i++){ //look at all three vertices of our v_b_op triangle
				if ((acc[b->sim[t->v_b_op][i]] == acc[t->v_a] || acc[b->sim[t->v_b_op][i]] == acc[t->v_b]) && !partial_match){
					partial_match = 1;
				}
				else if ((acc[b->sim[t->v_b_op][i]] == acc[t->v_a] || acc[b->sim[t->v_b_op][i]] == acc[t->v_b]) && partial_match){
					double_match = 1;
				}
			}
			if (double_match){
		...
		double_match = 0;
		for (int i = 0; i < 3; i++){ //look at all three vertices of our v_a_op triangle
			if ((acc[b->sim[t->v_a_op][i]] == acc[t->v_a] || acc[b->sim[t->v_a_op][i]] == acc[t->v_b_next]) && !partial_match){
				partial_match = 1;
			}
			else if ((acc[b->sim[t->v_a_op][i]] == acc[t->v_a] || acc[b->sim[t->v_a_op][i]] == acc[t->v_b_next]) && partial_match){
				double_match = 1;
			}
		}
		if (double_match){
		...
		if (!double_match) { //no double match. reset values and try with v_b_op
			...
			double_match = 0;
			for (int i = 0; i < 3; i++){ //look at all three vertices of our v_b_op triangle
				if ((acc[b->sim[t->v_b_op][i]] == acc[t->v_a] || acc[b->sim[t->v_b_op][i]] == acc[t->v_b_next]) && !partial_match){
					partial_match = 1;
				}
				else if ((acc[b->sim[t->v_b_op][i]] == acc[t->v_a] || acc[b->sim[t->v_b_op][i]] == acc[t->v_b_next]) && partial_match){
					double_match = 1;
				}
			}
			if (double_match){
		...
		double_match = 0;
		for (int i = 0; i < 3; i++){ //look at all three vertices of our v_a_op triangle
			if ((acc[b->sim[t->v_a_op][i]] == acc[t->v_a_next] || acc[b->sim[t->v_a_op][i]] == acc[t->v_b]) && !partial_match){
				partial_match = 1;
			}
			else if ((acc[b->sim[t->v_a_op][i]] == acc[t->v_a_next] || acc[b->sim[t->v_a_op][i]] == acc[t->v_b]) && partial_match){
				double_match = 1;
			}
		}
		if (double_match){
		...
		if (!double_match) { //no double match. reset values and try with v_b_op
			...
			double_match = 0;
			for (int i = 0; i < 3; i++){ //look at all three vertices of our v_b_op triangle
				if ((acc[b->sim[t->v_b_op][i]] == acc[t->v_a_next] || acc[b->sim[t->v_b_op][i]] == acc[t->v_b]) && !partial_match){
					partial_match = 1;
				}
				else if ((acc[b->sim[t->v_b_op][i]] == acc[t->v_a_next] || acc[b->sim[t->v_b_op][i]] == acc[t->v_b]) && partial_match){
					double_match = 1;
				}
			}
			if (double_match){
		...
		double_match = 0;
		for (int i = 0; i < 3; i++){ //look at all three vertices of our v_a_op triangle
			if ((acc[b->sim[t->v_a_op][i]] == acc[t->v_a_next] || acc[b->sim[t->v_a_op][i]] == acc[t->v_b_next]) && !partial_match){
				partial_match = 1;
			}
			else if ((acc[b->sim[t->v_a_op][i]] == acc[t->v_a_next] || acc[b->sim[t->v_a_op][i]] == acc[t->v_b_next]) && partial_match){
				double_match = 1;
			}
		}
		if (double_match){
		...
		if (!double_match) { //no double match. reset values and try with v_b_op
			...
			double_match = 0;
			for (int i = 0; i < 3; i++){ //look at all three vertices of our v_b_op triangle
				if ((acc[b->sim[t->v_b_op][i]] == acc[t->v_a_next] || acc[b->sim[t->v_b_op][i]] == acc[t->v_b_next]) && !partial_match){
					partial_match = 1;
				}
				else if ((acc[b->sim[t->v_b_op][i]] == acc[t->v_a_next] || acc[b->sim[t->v_b_op][i]] == acc[t->v_b_next]) && partial_match){
					double_match = 1;
				}
			}
			if (double_match){
...
void decompose(struct bfs_struct* b, struct rmq_struct* r, struct tripod_decomposition_struct* t, int sp, int f1, int f2, int f3, int* acc, int* acc2){
	...
	if (!stop){ //if there are more than one subproblems, carry on
		printf("working on subproblem %d\n", sp);
		printf("v_b = %d, v_c = %d, acc[v_b] = %d, acc[v_c] = %d\n", v_b, v_c, acc[v_b], acc[v_c]);
